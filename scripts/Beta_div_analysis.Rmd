...
```{r}
phylo16S_MTL <- phyloseq::subset_samples(bact, city =="Montreal")
...
ASV_spsum_1 <- otu_table(phylo16S_MTL) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phylo16S_MTL_spsum] %>%
  t() %>%
  as.data.frame()
ASV_spsum_1$ID <- rownames(ASV_spsum_1)
ASV_spsum_1 <- ASV_spsum_1 %>%
  mutate(class = if_else(ID %in% summer_mtl, "Summer", "Spring"))
ASV_spsum_1 <- filter_rows_same_last_part(ASV_spsum_1)

# Summarize to create one row per season
ASV_spsum_1 <- ASV_spsum_1 %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop") %>%
  column_to_rownames("class")

# --- Summer–Autumn comparison ---
ASV_sumau_1 <- otu_table(phylo16S_MTL) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phylo16S_MTL_sumaut] %>%
  t() %>%
  as.data.frame()
ASV_sumau_1$ID <- rownames(ASV_sumau_1)
ASV_sumau_1 <- ASV_sumau_1 %>%
  mutate(class = if_else(ID %in% summer_mtl, "Summer", "Autumn"))
ASV_sumau_1 <- filter_rows_same_last_part(ASV_sumau_1)
ASV_sumau_1 <- ASV_sumau_1 %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop") %>%
  column_to_rownames("class")
ASV_sumau_pa <- decostand(ASV_sumau_1, method = "pa")
ASV_sumau_rpa <- beta.multi(ASV_sumau_pa, index.family = "sorensen")
print(ASV_sumau_rpa)
...
ASV_spau_1 <- as.data.frame(ASV_spau_1)
ASV_spau_1$ID <- rownames(ASV_spau_1)

# Assign seasons BEFORE filtering
ASV_spau_1$class <- NA
ASV_spau_1$class[ASV_spau_1$ID %in% spring_mtl] <- "Spring"
ASV_spau_1$class[ASV_spau_1$ID %in% autumn_mtl] <- "Autumn"

# Remove rows not matched to any season
ASV_spau_1 <- ASV_spau_1 %>% filter(!is.na(class))

# Now filter to keep only samples with matching suffixes in both seasons
ASV_spau_1 <- filter_rows_same_last_part(ASV_spau_1)

# Double-check
table(ASV_spau_1$class)

# Summarize by season
ASV_spau_1 <- ASV_spau_1 %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop")

# Final formatting
rownames(ASV_spau_1) <- ASV_spau_1$class
ASV_spau_1$class <- NULL
library(vegan)
...
p1 <- ggplot2::ggplot(pa_data_long_r, aes(fill = beta, y = value, x = class)) + 
  ...
  scale_y_continuous(
    name = "Diversity dissimilarity value",
    limits = c(0, 0.4)
  ) +
  
  scale_x_discrete(labels = c(
    "Spring-Summer" = "Spring vs summer",
    "Summer-Autumn" = "Summer vs fall",
    "Spring-Autumn" = "Spring vs fall"
  )) +
...
# Create matrix from OTU table
ASV_spsum_sh <- t(as(otu_table(phylo16S_Sher), "matrix"))
...
# Assign seasons, summarize by class
ASV_spsum_sh1 <- ASV_spsum_sh1 %>%
  mutate(class = if_else(ID %in% spring_sh, "Spring", "Summer")) %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop")
rownames(ASV_spsum_sh1) <- ASV_spsum_sh1$class
ASV_spsum_sh1 <- as.data.frame(ASV_spsum_sh1)  # ensure it's a plain data.frame
ASV_spsum_sh1 <- column_to_rownames(ASV_spsum_sh1, "class")

# --- Summer–Autumn ---
ASV_sumau_sh <- otu_table(phylo16S_Sher) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phylo16S_SH_sumaut] %>%
  t() %>%
  as.data.frame()
ASV_sumau_sh$ID <- rownames(ASV_sumau_sh)
ASV_sumau_sh <- ASV_sumau_sh %>%
  mutate(class = if_else(ID %in% summer_sh, "Summer", "Autumn")) %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop")
ASV_sumau_sh <- as.data.frame(ASV_sumau_sh)
rownames(ASV_sumau_sh) <- ASV_sumau_sh$class
ASV_sumau_sh$class <- NULL

# --- Spring–Autumn ---
ASV_spau_sh <- otu_table(phylo16S_Sher) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phylo16S_SH_spaut] %>%
  t() %>%
  as.data.frame()
ASV_spau_sh$ID <- rownames(ASV_spau_sh)
ASV_spau_sh <- ASV_spau_sh %>%
  mutate(class = if_else(ID %in% spring_sh, "Spring", "Autumn")) %>%
  group_by(class) %>%
  summarise(across(where(is.numeric), sum), .groups = "drop")
ASV_spau_sh <- as.data.frame(ASV_spau_sh)
rownames(ASV_spau_sh) <- ASV_spau_sh$class
ASV_spau_sh$class <- NULL

# Presence/absence transformation
ASV_spsum_1_pa <- decostand(ASV_spsum_sh1, method = "pa")
ASV_sumau_shpa <- decostand(ASV_sumau_sh, method = "pa")
...
ASV_sumau_shrpa  <- beta.multi(ASV_sumau_shpa, index.family = "sorensen")
...
pa_plot_r2 <- rbind(
  ...
  dplyr::select(-beta.SOR) %>%
  mutate(class = c("Spring-Summer", "Summer-Autumn", "Spring-Autumn"))

# Reshape for plotting
pa_data_long_r2 <- gather(pa_plot_r2, beta, value, beta.SIM:beta.SNE, factor_key = TRUE)
...
p3 <- ggplot(pa_data_long_r2, aes(fill=beta, y=value, x=class)) + 
  geom_bar(position="stack", stat="identity", width = 0.5) + scale_fill_manual(name = "Total dissimilarity:",values = c("darkslateblue","grey"),
breaks = c("beta.SIM", "beta.SNE"),       
labels = c("Turnover component", "Nestedness component")) +
  ...
  scale_y_continuous(name="Diversity dissimilarity value", limits=c(0, 0.4)) +
   theme_bw() +
  theme(
    panel.border = element_blank(),
    panel.grid.major = element_blank(),
...
```{r}
phylo16S_qc <- phyloseq::subset_samples(bact, city =="Quebec")
...
ASV_spau_qc1 <- t(ASV_spau_qc1)
ASV_spau_qc1 <- as.data.frame(ASV_spau_qc1)
ASV_spau_qc1$ID <- rownames(ASV_spau_qc1)

# Assign class BEFORE summarising
...
p2 <- ggplot(pa_data_long_r1, aes(x = class, y = value, fill = beta)) +
  ...
  theme(
    ...
    axis.line = element_line(colour = "black", size = 0.8),
    axis.text.x = element_blank(),
    axis.text.y = element_text(size = 15, colour = "black"),  # ✔ show values
    axis.ticks.length = unit(0.4, "lines"),
    legend.position = "none",
    axis.title.y = element_blank(),  
    axis.title.x = element_blank(),
  ...
  scale_x_discrete(labels = c("Spring-Autumn" = "Spring vs fall")) +
  ggtitle("B  Québec")
p2
...
phyloITS_MTL <- phyloseq::subset_samples(fung, city == "Montreal")
...
pa_plot_rf1 <- rbind(
  ...
  dplyr::select(-c("beta.SOR"))
pa_plot_rf1$class <- c("Spring-Summer", "Summer-Autumn", "Spring-Autumn")

pa_data_long_rf23 <- gather(pa_plot_rf1, beta, value, beta.SIM:beta.SNE, factor_key = TRUE)
pa_data_long_rf23$class <- rep(pa_plot_rf1$class, each = 1)

p4 <- ggplot2::ggplot(pa_data_long_rf23, aes(fill = beta, y = value, x = class)) + 
  geom_bar(position = "stack", stat = "identity", width = 0.5) +
...
ASV_spsum_sh1 <- otu_table(phyloFungi_Sher) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phyloFungi_SH_spsum] %>%
  t() %>%
  as.data.frame()
ASV_spsum_sh1$ID <- rownames(ASV_spsum_sh1)
ASV_spsum_sh1 <- ASV_spsum_sh1 %>%
...
ASV_sumau_sh1 <- otu_table(phyloFungi_Sher) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phyloFungi_SH_sumaut] %>%
  t() %>%
  as.data.frame()
...
ASV_spau_sh1 <- otu_table(phyloFungi_Sher) %>%
  as("matrix") %>%
  t() %>%
  .[, colnames(.) %in% phyloFungi_SH_spaut] %>%
  t() %>%
  as.data.frame()
...
# Transform to presence-absence and calculate beta diversity
ASV_spsum_sh1_pa <- decostand(ASV_spsum_sh1, method = "pa")
ASV_sumau_shpa <- decostand(ASV_sumau_sh1, method = "pa")
ASV_spau_shpa <- decostand(ASV_spau_sh1, method = "pa")

ASV_spsum_sh_rpa <- beta.multi(ASV_spsum_sh1_pa, index.family = "sorensen")
...
pa_data_long_rf22 <- gather(pa_plot_rf, beta, value, beta.SIM:beta.SNE, factor_key = TRUE)
pa_data_long_rf22$class <- rep(pa_plot_rf$class, each = 1)


p6 <- ggplot(pa_data_long_rf22, aes(fill=beta, y=value, x=class)) + 
  geom_bar(position="stack", stat="identity",width = 0.5) + scale_fill_manual(name = "Total dissimilarity:",values = c("darkslateblue","grey"),
breaks = c("beta.SIM", "beta.SNE"),       
labels = c("Turnover component", "Nestedness component")) +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
```{r}
phyloITS_qc <- phyloseq::subset_samples(fung, city =="Quebec")
...
p5 <- ggplot(pa_data_long_r1f, aes(x = class, y = value, fill = beta)) +
  geom_bar(stat = "identity", position = "stack", width = 0.5) +
  scale_fill_manual(
    name = "Dissimilarity Component",
    values = c("darkslateblue", "grey"),
...
```{r}
# Subset fungal phyloseq object for Montréal
phyloplan_MTL <- phyloseq::subset_samples(plant, city == "Montreal")
...
pa_plot_rf25 <- rbind(
  ...
  dplyr::select(-c("beta.SOR"))
pa_plot_rf25$class <- c("Spring-Summer", "Summer-Autumn", "Spring-Autumn")

pa_data_long_rf26 <- gather(pa_plot_rf25, beta, value, beta.SIM:beta.SNE, factor_key = TRUE)
pa_data_long_rf26 <- pa_plot_rf25 %>%
  mutate(class = c("Spring-Summer", "Summer-Autumn", "Spring-Autumn")) %>%
  pivot_longer(cols = starts_with("beta."), names_to = "beta", values_to = "value")

p7 <- ggplot2::ggplot(pa_data_long_rf26, aes(fill = beta, y = value, x = class)) + 
  geom_bar(position = "stack", stat = "identity", width = 0.5) +
...
pa_data_long_rf23 <- gather(pa_plot_rf, beta, value, beta.SIM:beta.SNE, factor_key = TRUE)
pa_data_long_rf23$class <- rep(pa_plot_rf$class, each = 1)

p9 <- ggplot(pa_data_long_rf23, aes(fill=beta, y=value, x=class)) + 
  geom_bar(position="stack", stat="identity", width = 0.5) + scale_fill_manual(name = "Total dissimilarity:",values = c("darkslateblue","grey"),
breaks = c("beta.SIM", "beta.SNE"),       
labels = c("Turnover component", "Nestedness component")) +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
```{r}
phylo_plan_qc <- phyloseq::subset_samples(plant, city =="Quebec")
...
p8 <- ggplot(pa_data_long_r1, aes(x = class, y = value, fill = beta)) +
  geom_bar(stat = "identity", position = "stack", width = 0.5) +
  scale_fill_manual(
    name = "Dissimilarity Component",
    values = c("darkslateblue", "grey"),
  ...
  theme(
    ...
    axis.title.y = element_blank(),
    axis.title.x = element_blank(),  # <- This line removes the "class" label
    axis.ticks.length = unit(0.4, "lines"),
...
#Plot final
row1 <- (p1 | p2 | p3) + plot_layout(widths = c(1, 0.6, 1))
row2 <- (p4 | p5 | p6) + plot_layout(widths = c(1, 0.6, 1))
row3 <- (p7 | p8 | p9) + plot_layout(widths = c(1, 0.6, 1))

# Combine all
combined_plot_final <- row1 / row2 / row3 +
  plot_annotation(
    theme = ggplot2::theme(
      plot.title = ggplot2::element_text(size = 16, face = "bold", hjust = 0.5)
    )
